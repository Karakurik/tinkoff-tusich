...
object TodoSql {
  object sqls {
    val listAllSql: Query0[Todo] = sql"select * from TODOS".query[Todo]

    def findByIdSql(id: TodoId): Query0[Todo] =
      sql"select * from TODOS where id=${id.value}".query[Todo]

    def removeByIdSql(id: TodoId): Update0 =
      sql"delete from TODOS where id=${id.value}".update

    def insertSql(todo: CreateTodo): Update0 =
      sql"insert into TODOS (name, remainder_date) values (${todo.name.value}, ${todo.remainderDate.value
          .toEpochMilli()})".update

    def findByNameAndDate(name: TodoName, date: RemainderDate) =
  ...
  private final class Impl extends TodoSql {
    ...
    override def listAll: ConnectionIO[List[Todo]] =
      listAllSql.to[List]

    override def findById(
        id: TodoId
    ): ConnectionIO[Option[Todo]] =
      findByIdSql(id).option

    override def removeById(
        id: TodoId
    ): ConnectionIO[Either[TodoNotFound, Unit]] =
      removeByIdSql(id).run.map {
        case 0 => TodoNotFound(id).asLeft[Unit]
        case _ => ().asRight[TodoNotFound]
      }
      ...
      findByNameAndDate(todo.name, todo.remainderDate).option.flatMap {
        case None =>
          insertSql(todo)
            ...
            .map(id =>
              Todo(id, todo.name, todo.remainderDate).asRight[TodoAlreadyExists]
            )
